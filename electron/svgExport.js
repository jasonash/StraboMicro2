    ...
    try {
      ...
      if (child.placementType === 'affine') {
        try {
          ...
          if (!affineTileHash) {
            log.warn(`[SVGExport] Affine overlay ${child.id} missing affineTileHash`);
            continue;
          }
          ...
          if (!mediumBuffer) {
            log.warn(`[SVGExport] Affine medium image not found for ${child.id}`);
            continue;
          }
          ...
          log.info(`[SVGExport] Affine overlay ${child.id}: bounds=(${boundsOffset.x}, ${boundsOffset.y}), size=${transformedWidth}x${transformedHeight}`);
          ...
          log.info(`[SVGExport] Added affine overlay ${child.id} at (${compositeX}, ${compositeY})`);
        } catch (err) {
          log.error(`[SVGExport] Error processing affine overlay ${child.id}:`, err);
        }
        continue;
      }
