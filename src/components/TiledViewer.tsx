  ...
  ({ imagePath, onCursorMove }, ref) => {
    ...
    useEffect(() => {
      ...
      const loadImage = async () => {
        ...
        try {
          ...
          loadAllTiles(
            result.hash,
            result.metadata.tilesX,
            result.metadata.tilesY,
            result.fromCache,
            currentSession
          );
        } catch (error) {
    ...
    const loadAllTiles = useCallback(
      async (
        ...
        try {
          ...
          console.log(
            `[Session ${sessionId}] Loading all ${allTileCoords.length} tiles in background... (fromCache: ${fromCache})`
          );
          ...
          if (loadingSessionRef.current !== sessionId) {
            console.log(
              `[Session ${sessionId}] Session invalidated after tile fetch, skipping decode (tiles are cached for later)`
            );
    ...
    const calculateVisibleTiles = useCallback(() => {
      ...
      const endTileX = Math.min(
        tilesX - 1,
        Math.ceil((viewportX + viewportWidth) / TILE_SIZE) + padding
      );
      const startTileY = Math.max(0, Math.floor(viewportY / TILE_SIZE) - padding);
      const endTileY = Math.min(
        tilesY - 1,
        Math.ceil((viewportY + viewportHeight) / TILE_SIZE) + padding
      );
    ...
    const handleStageClick = useCallback(
      (e: any) => {
        ...
        if (!activeTool || activeTool === 'select') {
          ...
          const isSpot =
            (typeof targetName === 'string' && targetName.startsWith('spot-')) ||
              ...
              <>
                ...
                <div style={{ position: 'absolute', top: 0, left: RULER_SIZE, zIndex: 1000 }}>
                  <RulerCanvas
                    ...
                    position={position}
                    imageWidth={
                      activeMicrograph?.imageWidth || activeMicrograph?.width || imageMetadata.width
                    }
                    imageHeight={
